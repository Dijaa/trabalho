...
<div class="bg-white p-8 rounded shadow-md w-96">
    <h2 class="text-2xl font-bold mb-8 text-center">Documentos Pendentes</h2>
    <div class="grid grid-cols-1 gap-4 mt-8">
        ...
        <% List<FileModel> arquivos = FileDAO.buscarPendeteByIdAprovador(id); %>
        <% if (arquivos == null) { %>
        ...
        <div class="border border-gray-200 rounded p-4">
            ...
            <div>
                <a href="<%=request.getContextPath()%>/authArea/files/aproveOrDisaprove.jsp?id=<%= documento.getId() %>&aprove=0"
                   class="text-indigo-600 hover:text-indigo-500 hover:bg-gray-200 px-2 py-1 rounded">Reprovar</a>
            </div>
            <div>
                <a href="<%=request.getContextPath()%>/authArea/files/aproveOrDisaprove.jsp?id=<%= documento.getId() %>&aprove=1"
            ...
            <div>
                <a href="<%=request.getContextPath()%>/download?id=<%= documento.getId() %>"
                   class="text-indigo-600 hover:text-indigo-500 hover:bg-gray-200 px-2 py-1 rounded">Baixar
                    Documento</a>
            </div>
        </div>
        <% } %>
